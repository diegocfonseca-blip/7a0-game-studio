...
type Action =
  ...
  | { type: 'REAUCTION_ONLINE'; placements: Record<string, string>; rewards?: Record<number, number>; clubRewards?: Record<string, number>; champions?: Record<string, 'A' | 'B' | 'C' | 'D' | 'V'>; scorerValues?: Record<string, number>; copaChampion?: string | null; copaBrasilChampion?: string | null; supercopaChampion?: string | null; sponsorRewards?: Record<number, number>; sponsorResults?: Record<number, { tier: 1 | 2 | 3; brandId: string; hit: boolean; amount: number; floored?: boolean }>; stadiumOcc?: Record<number, number> } // carreira online: aplica acessos/quedas e refaz o LEILÃO (novo time), orçamento parelho
  | { type: 'OPEN_RESERVE_LIST'; placements: Record<string, string>; rewards?: Record<number, number>; clubRewards?: Record<string, number>; champions?: Record<string, 'A' | 'B' | 'C' | 'D' | 'V'>; scorerValues?: Record<string, number>; copaChampion?: string | null; copaBrasilChampion?: string | null; supercopaChampion?: string | null; mesmo?: boolean; sponsorRewards?: Record<number, number>; sponsorResults?: Record<number, { tier: 1 | 2 | 3; brandId: string; hit: boolean; amount: number; floored?: boolean }>; torcidaDeltas?: Record<string, number>; torcidaHist?: Record<string, { delta: number; motivo: string }[]>; stadiumOcc?: Record<number, number> } // carreira online: abre a tela de VENDA (listar pra leilão) já na temporada nova, antes da compra. mesmo=true → votou "mesmo time": mesma tela, SÓ decide contrato, sem mercado/leilão depois (vai pro CONFIRM_MESMO_TIME). sponsorRewards/Results = 🤝 aposta do patrocínio da temporada que ACABOU. torcidaDeltas = 🎪 quanto o torcidômetro de cada time humano mudou nesta temporada. torcidaHist = 🎪 chips do histórico sutil (motivo de cada mudança), pra guardar os últimos 6. copaBrasilChampion/supercopaChampion = 🏆🇧🇷🔵 critério próprio no ranking, só preenchido quando a Copa do Brasil está rolando
...
export function reducer(state: EscState, action: Action): EscState {
  ...
  switch (action.type) {
    ...
    case 'REAUCTION_ONLINE': {
      ...
      if (action.copaBrasilChampion) s.careerCopaBrasilHonors = { ...(s.careerCopaBrasilHonors ?? {}), [action.copaBrasilChampion]: (s.careerCopaBrasilHonors?.[action.copaBrasilChampion] ?? 0) + 1 } // 🏆🇧🇷 Copa do Brasil no histórico (critério próprio no ranking)
      if (action.supercopaChampion) s.careerSupercopaHonors = { ...(s.careerSupercopaHonors ?? {}), [action.supercopaChampion]: (s.careerSupercopaHonors?.[action.supercopaChampion] ?? 0) + 1 } // 🏆🔵 Supercopa no histórico (critério próprio no ranking)
