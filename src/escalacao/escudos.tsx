...
// 🔁 10/09 (madrugada): o Diego trocou pelo escudo DEITADO da prancha só-escudo —
// o estádio inteiro visto de cima (placas SANTA e PITÚ), mais largo que alto:
// 360x260 no arquivo, então width = size*360/260 (o render usa a ALTURA como base).
...
const arrudaEscudoRender = (size: number) => (
  <img src={arrudaEscudoImg} height={size} width={Math.round(size * 360 / 260)} alt="Tricolor do Arruda" style={{ flex: 'none', display: 'block', objectFit: 'contain' }} />
)
...
// 🐓🌙 Nightfull FC — 249x360 no arquivo, então a largura sai da PROPORÇÃO REAL
// (nunca width={size} chutado). Arte NOVA do dono (guilhermevictor539, 16/09):
// brasão alvinegro de moldura dourada, com o galo coroado de crista vermelha,
...
// 🦍🦂 Scorporila FC — 262x360 no arquivo, então a largura sai da PROPORÇÃO REAL
// (nunca width={size} chutado). Arte NOVA do dono (lucassrribeiroo2023, 16/09):
...
const remocadaEscudoRender = (size: number) => (
  <img src={remocadaEscudoImg} height={size} width={Math.round(size * 291 / 360)} alt="Remoçada" style={{ flex: 'none', display: 'block', objectFit: 'contain' }} />
)
...
// 🎙️ Fala D10 — 256x360 no arquivo, então a largura sai da PROPORÇÃO REAL (nunca
// width={size} chutado). Arte do dono (diegohdsf, 09/09): escudo preto e dourado
// com a caricatura do próprio dono, megafone, raios e coroa.
...
// 🐛 Bicho da Seda — 325x360 no arquivo, entao a largura sai da PROPORCAO REAL
// (nunca width={size} chutado). Arte NOVA do dono (davisantana1312/Davi, 09/09):
// o bicho-da-seda de oculos escuros enrolado nos fios, folhas de amoreira e o
// casulo, preto e branco-creme. Substituiu o escudo em SVG desenhado a mao (10/08).
...
// com creme e a gota. Substituiu o escudo do porco em SVG desenhado a mao (10/08).
// ⚠️ A prancha era VERDE em fundo VERDE: o recorte usou chave apertada (fundo tem
...
export const LOGOS_PRONTAS: Record<string, (size: number) => ReactNode> = {
  ...
  // 👑🦁 Rei da Bola FC (caiobegnamii, 16/09) — chega na Série A. As 4 formas do nome
  ...
  'Rei da Bola SC': reiDaBolaEscudoRender,
  ...
  // 🎙️ Fala D10 (diegohdsf) — Série A, no assento que era do Skyy FC (09/09), que
  // DESCEU pra Série D. As 4 formas do nome. ⚠️ SEM nome velho: o Skyy só mudou
  // de divisão. ⚠️ NÃO registrar "FalaD10" (sem espaço): é o nome de OUTRA conta
  // (fala.od10) — mimo é do e-mail, não da palavra.
  'Fala D10': falaD10EscudoRender,
  ...
  'Corporação Capsule SC': capsuleEscudoRender,
  ...
  // 🦁⚡ Remoçada (luiz.maia.luiz) — ARTE DO DONO, mandada pelo Diego em 18/09.
  // Brasão redondo azul-marinho do Leão do Remo de elmo alado, com as bandeiras do
  // Pará e do Brasil e a faixa "IMPONDO RESPEITO NO NORTE DO BRASIL".
  // 291x360 no arquivo, então a largura sai da PROPORÇÃO REAL (nunca width={size}).
  // 🧹 O escudo ANTIGO era SVG à mão aqui dentro (era pré-regra de peso); saiu de
  // vez e virou webp fora do bundle.
  'Remoçada': remocadaEscudoRender,
  'Remoçada FC': remocadaEscudoRender,
  'Remoçada EC': remocadaEscudoRender,
  'Remoçada SC': remocadaEscudoRender,
  ...
  // 🦍🦂 Scorporila FC (lucassrribeiroo2023) — ARTE NOVA em 16/09, mandada pelo dono.
  // O brasão alvinegro com o gorila-escorpião: cauda dourada segmentada por cima,
  // as pinças abraçando o escudo e o "SCORPORILA" em letra de raio.
  // As 4 formas do nome ficam presas ao mesmo escudo (regra dos 4 nomes reservados);
  // o nome velho (Realeza FC) resolve sozinho via newestTeamName.
  // 🧹 O escudo ANTIGO era SVG à mão aqui dentro, com versão MINI pra tabela (era
  // pré-regra de peso); saiu de vez e virou webp fora do bundle — o `objectFit:
  // contain` já dá conta do tamanho da tabela.
  'Scorporila': scorporilaEscudoRender,
  ...
  // 🟢 Xurupitas FC (denilson.stifler10/Denis — coração PALMEIRAS). 09/09: o dono
  ...
  // 🐛 Bicho da Seda (davisantana1312/Davi — coração BOTAFOGO). 09/09: o dono
  // mandou arte própria (escudo + mascote + camisa) e ela SUBSTITUIU o escudo em
  // SVG desenhado à mão de 10/08 (mariposa). As 4 formas do nome; o nome velho
  // (Red Bull Diet) segue no OLD_NAME do data.ts.
  'Bicho da Seda': bichoDaSedaEscudoRender,
  'Bicho da Seda FC': bichoDaSedaEscudoRender,
  'Bicho da Seda EC': bichoDaSedaEscudoRender,
  'Bicho da Seda SC': bichoDaSedaEscudoRender,
  // 🐓🌙 Nightfull FC (guilhermevictor539) — ARTE NOVA em 16/09, mandada pelo dono.
  // O galo alvinegro coroado, com a crista vermelha e a lua, no brasão listrado de
  // moldura dourada. As 4 formas do nome ficam presas ao mesmo escudo (regra dos 4
  // nomes reservados). O escudo ANTIGO era SVG à mão aqui dentro, com até uma
  // versão MINI pra tabela (era pré-regra de peso); saiu de vez e virou webp fora
  // do bundle — o `objectFit: contain` já dá conta do tamanho da tabela.
  'Nightfull': nightfullEscudoRender,
  'Nightfull FC': nightfullEscudoRender,
  'Nightfull EC': nightfullEscudoRender,
  'Nightfull SC': nightfullEscudoRender,
  ...
  // As 4 formas do nome ficam presas ao mesmo escudo (regra dos 4 nomes).
  // 🧹 O escudo ANTIGO era SVG desenhado à mão aqui dentro (nasceu em 09/08, antes
  // da regra de peso); saiu de vez e virou webp FORA do bundle.
  'La Bestia Negra': bestiaEscudoRender,
  'La Bestia Negra FC': bestiaEscudoRender,
  'La Bestia Negra EC': bestiaEscudoRender,
  'La Bestia Negra SC': bestiaEscudoRender,
}
