...
export async function heartbeat(mode: 'cpu' | 'online' | 'career', displayName: string | undefined, screen: string, career?: { season: number; division: string }, deckLeague?: 'br' | 'eu') {
  try {
    const { data } = await supabase.auth.getUser()
