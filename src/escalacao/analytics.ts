...
export async function heartbeat(mode: 'cpu' | 'online' | 'career', displayName: string | undefined, screen: string, career?: { season: number; division: string; coins?: number; titles?: number }, deckLeague?: 'br' | 'eu' | 'both' | 'todos') {
  try {
    const user = await authUser()
