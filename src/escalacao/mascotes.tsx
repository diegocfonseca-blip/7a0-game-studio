...
export const MASCOTES: Record<string, ReactNode> = {
  ...
  // 🔁 10/09 (madrugada): a COBRA DA DINAMITE — boné tricolor, isqueiro, cartucheira,
  // bananas de dinamite e fogo no chão. 304x440 no arquivo.
  // 🧼 A 1ª prancha vinha com a cobra POR CIMA do escudo e da camisa (fumaça e cauda
  // sobrepostas, sem recorte honesto). O Diego pediu a arte SOZINHA e mandou de novo —
  // é esta. Fundo verde-ESCURO desta vez: a chave é por DISTÂNCIA da cor do fundo
  // (medida: fundo a ≤8 de distância, arte a ≥28), não pelo verde puro. Como o fundo
  // escuro TINGE o contorno preto do desenho (fio verde-musgo em volta), todo pixel em
  // que o verde lidera vira neutro — a arte não tem verde nenhum. Zero perda.
  cobra_arruda: (
    <img src={arrudaCobraImg} height={176} width={Math.round(176 * 304 / 440)} alt="Cobra Coral — Tricolor do Arruda FC" style={{ flex: 'none', display: 'block', objectFit: 'contain' }} />
  ),
  ...
  // 😇 Marcos — São Marcos Antônio FC (marcomak03, 13/09). 297x440 no arquivo,
  // então a largura sai da PROPORÇÃO REAL (nunca width={size} chutado). A mascote é o
  // próprio Marcos: o goleiro de auréola ajoelhado apontando pro céu, com o porquinho
  // de auréola do lado.
  saomarcos_porco: (
  ...
  theuzudo_morcego: (
    <img src={theuzudoMascoteImg} height={176} width={Math.round(176 * 264 / 351)} alt="Theuzinho — Theuzudo FC" style={{ flex: 'none', display: 'block', objectFit: 'contain' }} />
  ),
  nata_palhaco: (
...
export const CARIMBO_GOL: Record<string, string> = {
  ...
  // 🐦‍⬛ a Gralha carimba o placar (danielmanfre5, 30/08). As 4 formas do nome
  // + o nome VELHO: carreira antiga com o Livre-pool acompanha o batismo.
  'Manfré FC': 'manfre_gralha',
  'Manfré': 'manfre_gralha',
  ...
  // 🐓 Briga de Galo FC — as 4 formas do nome e a CAIXA ALTA (sem nome velho: o
  // Vasco da Grana só mudou de divisão, continua no jogo)
