...
export interface QuickCopaState {
  ...
  legIdx: 0 | 1 | 2      // ⚽ perna sendo jogada (final usa só a 0 — jogo único) · 🏀 jogo 1/2/3 da série melhor de 3
...
export interface EscState {
  ...
  careerCronica?: Record<string, { t: number; div: string; campeao?: 1; copa?: 1; sup?: 1 }[]> // 📼 MEMÓRIA DO JORNAL (Diego 24/08): uma linha POR TEMPORADA por clube humano (chave m<id>) — divisão jogada, título da divisão, Copa, Supercopa. É daqui que o jornal puxa manchete de história ("3º título seguido", "acabou o jejum"). Começa a gravar a partir de agora (o passado não tem como reconstruir); teto de 200 temporadas.
  ...
  careerCrise?: Record<number, { playerId: string; playerName: string; pos: Sector }> // 🚨 crise ativa aguardando decisão do técnico (mgrId → jogador que anunciou saída). Some ao resolver (RESOLVE_CAREER_CRISE) — vira folclórico de graça ou Cria da Base, sem custo.
  ...
  careerSubMode?: 'dinamico' | 'intervalo' // 🔁 SÓ CARREIRA OFFLINE (Diego 12/08): como o técnico faz troca. 'dinamico' (padrão) = como sempre foi (troca vale pro PRÓXIMO jogo, sem pausa). 'intervalo' = o jogo PARA aos 45' e abre o banner pra trocar SÓ pro 2º tempo. Preferência do técnico, NÃO zera entre temporadas.
  careerHalftime?: Record<number, Record<number, { xi2: string[]; formation?: FormationKey; tactic?: Tactic }>> // 🔁 decisão de intervalo POR RODADA (mgrId → rodada 0-based → time do 2º tempo). xi2 = os 11 ids que entram no 2º tempo (no máx. 3 diferentes do 1º tempo, mesma posição/formação — validado no reducer). Só vale pro 2º tempo daquele jogo; NÃO muda o time do próximo (isso é no careerLineup). Zera entre temporadas. Motor re-simula o 2º tempo com sub-rng ISOLADO (não cascateia em outros jogos).
  careerPenalty?: Record<number, Record<number, { scored: boolean; taker: string }>> // ⚽ PÊNALTI DECISIVO POR RODADA (mgrId → índice do jogo 0-based → resultado). SÓ CARREIRA OFFLINE. Aparece 0-2x/temporada em jogo decisivo de última hora (um gol empata ou vira). `taker` = id do cobrador escolhido; `scored` = converteu. Motor SÓ soma 1 gol ao time do humano naquele jogo se scored (min 90), sem rng — resultado já decidido pelo jogador. Vazio = simulação BYTE-IDÊNTICA (não afeta ninguém). Zera entre temporadas.
  ...
  copaMode?: 'liga' | 'liga_copa' | 'liga_liberta' | 'liga_champions' | 'champions' // 'champions' = SÓ Champions (25/09): sem liga, direto na tabela de 36 — só no rápido offline
  ...
  // 🧾 O RECIBO DOS SEUS TÍTULOS DE COPA (16/08 — relato "ganhei a Copa do Brasil
  // e não contou"). Lista das TEMPORADAS em que VOCÊ foi campeão. Existe porque
  // um contador solto não sabe se um número está inflado (título contado duas
  // vezes) ou furado (título perdido) — e a "cura" que tentava adivinhar isso
  // APAGAVA título de verdade (medido: 3 Copas viravam 0 só de continuar a
  // carreira). Com a lista, a conta é o tamanho dela: não duplica (a temporada
  ...
  // 🧢 SETOR TÉCNICO (27/08, a forma FINAL — palavras do Diego: "é idêntico ao
  // leilão normal, só vem antes do goleiro; é como uma posição nova apenas"):
  // quando você alicia um técnico, ele vira este lote e o pregão REAL abre com
  // o setor Técnico — mesma tela, mesmo envelope cego, mesmo martelo. Ninguém
  // cobriu? Ele fica onde está. null/ausente = leilão normal de sempre.
  tecLote?: { nome: string; clube: string | null; piso: number } | null
