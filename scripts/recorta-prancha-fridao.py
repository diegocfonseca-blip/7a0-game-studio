#!/usr/bin/env python3
# 🐴 RECORTE DA PRANCHA DO FRIDÃO FC (felipe.ofrida, 16/09).
#
...
#
# Rodar: python3 scripts/recorta-prancha-fridao.py <prancha.png>
import sys, numpy as np
from PIL import Image

SRC = sys.argv[1] if len(sys.argv) > 1 else '/root/.claude/uploads/fridao.png'
...
for i in tres:
    ...
    for j in range(1, n + 1):
        ...
        if ys.start <= cy < ys.stop and xs.start <= cx < xs.stop: m |= (lab == j)
    mascaras.append(m)
...
salva(escudo, 'src/escalacao/img/fridao-escudo.webp', 360, 30)
salva(mascote, 'src/escalacao/img/fridao-mascote.webp', 440, 45)
salva(camisa, 'scripts/kits/fridao-camisa.webp', 620, 200)
