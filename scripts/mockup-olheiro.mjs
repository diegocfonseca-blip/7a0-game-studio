...
//   2. a mesma janelinha pra quem NÃO tem Olheiro (só a porta)
...
const linha = (nome, estado, fame) => {
  ...
  const apagado = estado === 'preso' || estado === 'lenda' || estado === 'teto'
  const bg = estado === 'preso' || estado === 'lenda' ? '#eee' : estado === 'marcado' ? '#E9F9EF' : '#fff'
  const borda = estado === 'preso' || estado === 'lenda' ? 'transparent' : GREEN
  const rot = estado === 'lenda' ? '🔒 lenda · só o 👑 Lenda sonda' : estado === 'preso' ? `📝 falta${fame > 1 ? 'm' : ''} ${fame}` : estado === 'marcado' ? '✔ no leilão · tirar' : estado === 'teto' ? '🔒 já sondou 1' : '🆓 + sondar'
...
const caixaJogadores = (tier) => `
  <div style="border:3px dashed ${INK};border-radius:14px;padding:9px 10px;margin-top:11px;background:#FBF6E8">
    <p style="margin:0 0 5px;display:flex;align-items:center;gap:6px">
      <span style="${OSW};font-weight:900;font-size:9.5px;text-transform:uppercase;letter-spacing:.06em;color:${INK};background:${tier === 'ouro' ? OURO : PRATA};border:2px solid ${INK};border-radius:999px;padding:2px 8px">${tier === 'ouro' ? '👑 Olheiro Lenda' : '⭐ Olheiro Craque'}</span>
    ...
    ${linha('Naldo', 'teto')}
    ${posTit('Meia')}
    ${linha('Djalminha', 'teto')}
    ${linha('Zico', tier === 'ouro' ? 'teto' : 'lenda')}
    ${posTit('Atacante')}
    ${linha('Túlio Maravilha', 'preso', 4)}
...
const TXT_SEM = `Toque num clube e marque o <b>técnico</b> que você quer — <b>máx. 1 por temporada</b>, e só quem está <b>🆓 sem contrato</b>. É igual listar pra venda, só que ao contrário: <b>o sondado vai pro leilão</b> — ele abre o pregão como uma posição a mais, <b>antes dos goleiros</b>. 🔨 Sondar <b>jogador</b> é do 🕵️ <b>Olheiro</b> (⭐ Craque sonda de craque pra baixo · 👑 Lenda sonda tudo).`
...
const apoie = () => `
  <div style="border:3px solid ${INK};border-radius:16px;box-shadow:4px 4px 0 ${INK};overflow:hidden;background:#fff">
    ...
    <div style="padding:8px 12px 12px">
      ...
      ${item('② <b>Sondar jogador</b> de outro clube na janela antes do leilão: o sondado <b>vai pro pregão</b> e você briga por ele — 1 por leilão, igual ao técnico.', 2)}
  ...
  <div style="border:3px solid ${INK};border-radius:12px;overflow:hidden;margin-top:14px;box-shadow:4px 4px 0 ${INK};background:#fff">
    ...
    <table style="width:100%;border-collapse:collapse;font-size:9.5px;font-weight:900">
      ...
      ${[['Modo Manual (ritmo)', '—', '✓', '✓', '✓'], ['🕵️ Olheiro (nível + sondar)', '—', 'até craque', 'até lendas', 'até lendas'], ['Carreiras salvas', '2', '4', '6', '6'], ['Grupo VIP no zap', '—', '✓', '✓', '✓'], ['Carreira Online e Ligas', '—', '—', '✓', '✓']]
...
<div style="display:flex;gap:22px;align-items:flex-start">
  ...
  ${tela('2 · janelinha do clube · quem NÃO tem Olheiro', abaSondar(TXT_SEM) + rodape() + janela(cartaTecnico() + porta('🕵️ Quer sondar <b>JOGADOR</b> deste clube também? É do <b>Olheiro</b>: ⭐ Craque sonda de craque pra baixo · 👑 Lenda sonda TUDO — <u>toca aqui</u>')), 'Sem Olheiro a lista de jogadores nem aparece: só o técnico (igual hoje) e a <b>porta</b> pontilhada, no lugar da dor, abrindo a loja do Apoie.')}
  ...
  ${tela('4 · Apoie · card ⭐ Craque e a tabela', apoie(), 'O benefício ganhou nome: <b>🕵️ Olheiro Craque</b>, em lista (① nível · ② sondar jogador), com o exemplo do Túlio pra sondar. O <b>📲 Grupo VIP</b> desceu do Lenda pro Craque. No Lenda vira "Olheiro Lenda: tudo, lenda inclusive" (Romário 93–99 + Zico pra sondar). A tabela mostra os dois.')}
</div>`
