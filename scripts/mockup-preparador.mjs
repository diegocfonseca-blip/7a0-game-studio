...
// Nomes fechados com ele: 🟢 Rui Faria (100) · ⭐ Paulo Paixão (500) · 👑 Paco
// Seirulo (1000). Salário = 10% do preço por temporada (a MESMA regra do técnico).
...
const PREP = { emoji: '🏋️', papel: 'PREPARADOR FÍSICO', nome: 'Rui Faria', sub: '🟢 Bom · banco devolve +6', valor: 100, grad: G_VERDE, contrato: 'faltam 5 temporadas' }

// ── a lojinha do preparador (o que abre no CONTRATAR) ───────────────────────
const LOJA = [
  { grad: G_VERDE, selo: '🟢', cat: 'BOM', nome: 'Rui Faria', bio: 'Começou como preparador físico do Mourinho e virou auxiliar dele no Porto, Chelsea, Inter, Real e United.', preco: 100, banco: 6, seguidas: 4, cor: '#1E7A45' },
  { grad: G_PRATA, selo: '⭐', cat: 'CRAQUE', nome: 'Paulo Paixão', bio: 'Preparador físico da Seleção Brasileira em quatro Copas do Mundo seguidas.', preco: 500, banco: 8, seguidas: 5, cor: '#6C7787' },
  { grad: G_OURO, selo: '👑', cat: 'LENDA', nome: 'Paco Seirulo', bio: 'Décadas no Barcelona — a preparação física por trás da era mais vitoriosa do clube.', preco: 1000, banco: 12, seguidas: 8, cor: '#9A7400' },
]
const cardLoja = (p) => `<div class="lj" style="background:${p.grad}">
  <div class="ljin">
    <span class="ljc">${p.selo} ${p.cat}</span>
    <span class="ljn">${p.nome}</span>
    <span class="ljb">${p.bio}</span>
    <span class="ljr"><b>+${p.banco}</b> de gás por rodada no banco<br><i>o titular joga ${p.seguidas} seguidas sem nunca cansar</i></span>
 ...
 *{box-sizing:border-box}
 body{margin:0;background:${CREME};font-family:system-ui;color:${INK};width:1380px;padding:30px}
 ...
 .ljg{display:flex;gap:14px;padding:16px;background:${CREME}}
 .lj{flex:1;border:4px solid ${INK};border-radius:16px;box-shadow:4px 4px 0 ${INK};padding:4px}
 ...
 .ljn{display:block;font-family:Oswald;font-weight:900;font-size:24px;line-height:1.05;margin-top:1px}
 .ljb{display:block;font-size:11.5px;font-weight:700;color:#4a4636;line-height:1.45;margin:6px 0 9px;min-height:50px}
 .ljr{display:block;font-size:12px;font-weight:800;color:${INK};background:#F1F7F2;border-left:4px solid ${GREEN};border-radius:6px;padding:7px 9px;line-height:1.4}
 .ljr i{font-style:normal;font-weight:700;color:#5a5647;font-size:10.5px}
...
<div class="nota">
  ...
  · <b>Salário igual ao do técnico</b>: 10% do preço, por temporada (100 → 10 · 500 → 50 · 1000 → 100), e entra na <b>Folha do Time</b>.<br>
...
const p = await b.newPage({ viewport: { width: 1380, height: 900 }, deviceScaleFactor: 2 })
