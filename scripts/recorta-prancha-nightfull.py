...
# Saída em /tmp/nightfull/, com as provas sobre fundo CREME (nunca branco).
from PIL import Image
import numpy as np
from scipy import ndimage
import os

PRANCHA = '/root/.claude/uploads/15782737-58e2-54d9-971e-653cb64061f2/888887df-image.png'
VERDE = (2, 249, 61)
os.makedirs('/tmp/nightfull', exist_ok=True)
...
def prova(img, nome):
    ...
    b.save(f'/tmp/nightfull/{nome}-prova.png')


def salva(img, nome, um_so=True):
    out = limpa_franja(poeira_fora(tira_verde(img), um_so=um_so))
    out.save(f'/tmp/nightfull/{nome}.png'); prova(out, nome)
    w, h = out.size
