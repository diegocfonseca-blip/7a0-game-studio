...
# Saída em /tmp/saoluiz/, com as provas sobre fundo CREME (nunca branco).
from PIL import Image
import numpy as np
from scipy import ndimage
import os

PRANCHA = '/root/.claude/uploads/15782737-58e2-54d9-971e-653cb64061f2/c1e11d0c-image.png'
VERDE = (2, 249, 61)
os.makedirs('/tmp/saoluiz', exist_ok=True)
...
m = {'escudo': np.zeros_like(obj), 'camisa': np.zeros_like(obj)}
for i in range(n):
    if t[i] < 200:
        continue
    x = cx[i][1]; meio = (x.start + x.stop) / 2
    alvo = 'escudo' if meio < 760 else 'camisa'
    m[alvo] |= (lab == i + 1)
    if t[i] > 3000:
