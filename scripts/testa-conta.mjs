...
const clubes = [...conta.match(/const CLUBES_PRINCIPAIS = \[(.*?)\]/)[1].matchAll(/'([^']+)'/g)].map(m => m[1])
assert.equal(clubes.length, 12)
assert.deepEqual(clubes, [...clubes].sort((a, b) => a.localeCompare(b, 'pt-BR')))
assert.ok(conta.indexOf('<option value="__outros__">') > conta.indexOf('{CLUBES_PRINCIPAIS.map'))
assert.ok(conta.includes('outroClube && !clube') && conta.includes('maxLength={60}'))
assert.ok(conta.includes('time_coracao: clube') && !conta.includes("time_coracao: '__outros__'"))
console.log('Preferência: lista curta/alfabética, Outros no fim, validação e gravação do nome real OK.')
