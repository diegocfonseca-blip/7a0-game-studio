...
create table public.esc_copa_clock_preview (
 ...
 step integer not null default 0 check(step between 0 and 12),
 running boolean not null default false,
 manual boolean not null default false,
 speed numeric not null default 1 check(speed in (0.25,0.5,1,2,4)),
 started_at timestamptz not null default now(),
 updated_at timestamptz not null default now(),
 duration_ms integer not null default 14000,
 extra_ms integer not null default 700 check(extra_ms between 700 and 60700),
 primary key(room_id,edicao,seed)
);
alter table public.esc_copa_clock_preview enable row level security;
revoke all on public.esc_copa_clock_preview from public,anon,authenticated;
grant select,insert,update on public.esc_copa_clock_preview to authenticated;
create policy preview_read on public.esc_copa_clock_preview for select to authenticated using (
 (select auth.jwt()->>'email') in ('diego.c.fonseca@gmail.com','diego.c.fonseca2@gmail.com') and
...
create policy preview_insert on public.esc_copa_clock_preview for insert to authenticated with check (
 (select auth.jwt()->>'email') in ('diego.c.fonseca@gmail.com','diego.c.fonseca2@gmail.com') and
 exists(select 1 from public.game_rooms g where g.id=room_id and g.host_id=(select auth.uid()))
);
create policy preview_update on public.esc_copa_clock_preview for update to authenticated using (
 (select auth.jwt()->>'email') in ('diego.c.fonseca@gmail.com','diego.c.fonseca2@gmail.com') and exists(select 1 from public.game_rooms g where g.id=room_id and g.host_id=(select auth.uid()))
) with check (
 (select auth.jwt()->>'email') in ('diego.c.fonseca@gmail.com','diego.c.fonseca2@gmail.com') and exists(select 1 from public.game_rooms g where g.id=room_id and g.host_id=(select auth.uid()))
);
create function public.esc_copa_preview_clock(p_room uuid,p_edicao integer,p_seed bigint,p_command text default 'read',p_revision bigint default -1,p_speed numeric default 1,p_extra integer default 0)
returns jsonb language plpgsql security invoker set search_path='' as $$
declare r public.esc_copa_clock_preview; stamp timestamptz := clock_timestamp(); target integer;
begin
 if coalesce(auth.jwt()->>'email','') not in ('diego.c.fonseca@gmail.com','diego.c.fonseca2@gmail.com') then raise exception 'Preview not allowed' using errcode='42501'; end if;
