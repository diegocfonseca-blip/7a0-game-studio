-- 🌍 COPA DO MUNDO COM OITAVAS (19/09) — o relógio da sala acompanha os passos novos.
--
-- Formato de Copa de 24: 6 grupos de 4 (3 rodadas), passam 16 → oitavas, quartas,
-- semi e final em jogo único. Passos: 1–3 rodadas · sorteio 4 · oitavas 5 ·
-- quartas 6 · semi 7 · final 8 · cerimônia 9. Bola rolando em 1–3 e 5–8.
-- (Antes, do mesmo dia: 5 rodadas + quartas/semi/final = 10 passos, bola em 1–5 e 7–9.)
--
-- ⚠️ Estes números são a CÓPIA de `src/escalacao/copa-passos.ts` (PASSO_COPA).
-- Mudou lá, muda aqui. Só duas linhas mudam: `r.step<9` e os dois `between`.
...
begin
 ...
 if p_command not in ('read','init') then
  ...
  -- 🌍 oitavas + jogo único: 9 passos; bola rolando nas 3 rodadas de grupo e em oitavas/quartas/semi/final (5–8)
  elsif p_command='next' and not r.running and r.step<9 then
   target:=r.step+1; r.step:=target; r.running:=target between 1 and 3 or target between 5 and 8;
