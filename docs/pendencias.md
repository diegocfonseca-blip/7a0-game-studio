...
- **Etapa 5 terminada** em `lobby.tsx` (passou a importar `tr`/`getLang` de `lang.ts`): chat da espera, Bafo (escolha de carreira), entrar/cadastrar/nova senha, menu (chame a galera, logado como, partida em andamento, minhas ligas com editar/excluir/convidar, minhas carreiras), criar sala v1 e v2 (modos, explicações, quadro da liga pra Lenda e pra não-Lenda, baralho, duplas, rivais, depois da liga, ritmo, sala/senha/chat/stream/tempo), lista de salas (selos, linha de detalhes, grupo do zap), entrar com código, continuar carreira, modo stream, senha da sala, sala de espera (código, convite, faixa da liga com remarcar/regras, Copa do Mundo, aviso do Bafo, lista de técnicos com duplas inteiras, zoeira e buzinas, gente montando, botão de começar/aguardando, sair/excluir, host saiu) e **todas as mensagens de erro/confirm** (login, criar sala, senha, ligas, carreira, duplas, Copa). Texto do convite compartilhado também sai em EN quando o botão está em EN.
- **Etapa 6 (3ª leva do dia) — feita**: janela de conta (`conta.tsx`) e `NOME_MSG` (nome em uso/batismo, agora Proxy que lê o idioma); **home** (a de todo mundo, a nova e a ilustrada), Minhas Carreiras, Manual do Técnico, Dúvidas de quem joga (FAQ inteiro reescrito em EN), nota da conta e novidades da home, menu fixo; **LigaHub** (rank/estante/temporadas/ajustes/regras + zoeira do mico por sentido); **Copa do Mundo** da carreira (`copa-mundo.tsx`: portão, escolha, convocação, torneio ao vivo, cerimônia, mural, estatísticas) e **online** (`copa-mundo-online.tsx`: escolha na sala, painel do dono, estante, fila da bandeira, banner, convocação); faixa "versão nova" (`aviso-versao.tsx`); botões "em breve" da home (`careeronline.tsx`); tela de senha nova; **boas-vindas ao pregão** (`EscStreamIntro`), **Monte final** (com as cutucadas), **Cerimônia da Revelação**, **Meu Álbum**, **Ranking** e o perfil do técnico (documentos, clube do coração, sala de troféus, álbum). Home conferida na tela em EN (`en-home.png`), sem erro.
- **Etapa 6 (4ª leva do dia) — varredura final do que sobrou, feita**: chat da partida online (`ChatWidget`) e rodapé da tela (`Shell`: sair/voltar pro menu/gerenciar técnicos/remover); **assistente novo da carreira** (clube › presidente › pregão, roupas do presidente); fim de temporada da carreira clássica (subiu/rebaixado/continua, salvar/sair, modal "salvar carreira" com cadastrar/entrar e erros) e o cartão "carreira em andamento" da home; balanço do pregão (achado/mico/mão furada/mão de vaca/perna-de-pau — pelo sentido), dica do auge, "o relógio não para", "Titulares", "há X dias"; histórico da torcida (motivo gravado em PT no save, `motivoTorcida()` traduz só ao mostrar), cabeçalho do pênalti, prévia do apoio, bi/tri/tetra da Série A; **`store.tsx` passou a importar `tr`/`getLang`**: avisos da sala online (abriu em outro aparelho, virou host, manda o lance de novo, expulso, faixa do dono sumido) e o giro da liga (Copa dos 8/Libertadores/mercado liberado — gravado no idioma da hora); **estádio/patrocínio**: `estadiodata.ts` ganhou `sectorNome/extraNome/extraReq/extraPerk/sponsorBetMeta` (chaves e números iguais, só o rótulo troca) e o apelido do estádio em EN — usados em `estadio.tsx`, `pyramidseason.tsx`, `career-sponsor-visual.tsx` e `career-stadium-model.ts`; **extrato**: rótulo gravado no save em PT → `ledgerLabel()` traduz na leitura; **novidades da home**: campo opcional `en` por linha (`novTitulo/novTexto`), as 14 mais recentes já traduzidas — **regra nova: toda novidade nasce com `en`** (sem `en` a tela em EN mostra o PT, não quebra); `index.tsx`: aviso "jogando dentro do app", tela de erro, faixa "sessão caiu", página de parar e-mails. `jornal-sala.tsx` já era bilíngue (conferido).
- ⚠️ **O que AINDA está em PT, e por quê**:
  - 💛 **Tela do APOIE inteira** (`ApoieButton` em `screens.tsx`, ~300–880: pacotes, preços em R$, Pix, história do Diego e do Luca) — **não traduzi de propósito**: o pagamento é Pix/Mercado Pago em reais, só funciona pra brasileiro; traduzir o texto sem traduzir o meio de pagar dá promessa falsa pra gringo. **Decidir com o Diego** se quer (a) deixar em PT, (b) traduzir mesmo assim, ou (c) uma versão curta em EN dizendo que o apoio é só via Pix (Brasil).
  - 🎙️ **Texto criativo gerado**: falas do pênalti decisivo (`PEN_INTRO/PEN_GO/PEN_DE/PEN_FO` e legendas), manchetes da carreira (`jornal.tsx`), `historiaSondagem`, `eventoTituloBanner`, textos das comissões da agência (`r.texto` nasce no store), `marketLog`, eventos/crises (`eventos.ts`), **bios das cartas** (`data.ts`) — tudo zoeira/humor BR. **Perguntar ao Diego** se traduz pelo sentido (como fiz no O Martelo e no mico) ou se fica em PT como "tempero".
  - 🔒 Telas que só o Diego vê (não aparecem pro jogador): `dinastia.tsx` (Modo Manager), `salao.tsx` (Salão dos Batismos, prévia), a tela velha de teste da Carreira Online (`careeronline.tsx` abaixo dos botões), painel do admin.
...
- **Etapa 3 (temporada + fim de jogo) — feita e conferida na tela** (jogo rápido inteiro rodado em EN, prints `en-temporada.png`/`en-fim.png` do scratchpad): barra da rodada e colocação (`1st/2nd/3rd` via `ordinal()` novo em `lang.ts`), controles (próxima rodada/pular/manual/auto/velocidade, cadeado do Modo Manual), placar ao vivo (selos de gol, apito inicial/intervalo/final — **mesma quantidade de frases nos dois idiomas**, pro sorteio por `roundKey` bater igual na sala toda), táticas (`Park the bus/Balanced/Attack`), manchetes pessoais, clássico/retrospecto, folga, Copa dos 8 e Libertadores ao vivo (fases, ida/volta, agregado, regulamento), fase de grupos da Libertadores, fim de jogo (colocação, resumo liga+copa, quadros de abertura, bafo amistoso, reiniciar/nova temporada/novo pregão, "continuar com esse time", compartilhar, votação "E agora?" inteira), visual online (`online-match-visual.tsx`: VOCÊ/RIVAL, ritmo, abas, pênaltis compactos) e **O Martelo** (edição da sala e do torneio, tela + imagem compartilhada — notas da redação traduzidas pelo SENTIDO; o PT ficou idêntico).
- **Etapa 4 (carreira `pyramidseason.tsx`) — a maior parte visível**: tabelas das séries, artilharia/garçons, barra de abas e faixa do topo, cabeçalho por competição, controles, tela de desfecho (campeão/acesso/queda/ficou), avisos (Copa do Brasil, Supercopa, "a sua Copa", TV, cota extra, suspenso, escalação com buraco), decisão de fim de temporada (solo e votação online), multiclubes, sub-abas do Clube, Bico de Folga, Rank/Elenco, janela de contratos e "listar pra leilão" (contratos por jogador, renovar em massa, escada de categorias, avisos de venda), banners de evento/crise/intervalo, rótulos de posição (`POS_LABEL` virou Proxy que lê o idioma). Também: rodapé do jogo, botão de apoio, "sair do jogo".
- Técnica: `tr(pt, en)` novo em `lang.ts` pra usar fora de componente (jornal, funções puras). Os helpers `LE`/`LS` do `screens.tsx` deixaram de exigir basquete.
- ⚠️ **O que AINDA está em PT** (próximas etapas): **manchetes geradas** (giro da rodada, notícias da carreira, `eventoTituloBanner`/`historia`, narração do pênalti decisivo — texto criativo gerado no `store.tsx`/`pyramidseason.tsx`); **carreira**: `FinancasTab`, `EscritorioTab`/Agência, `RankingTab`, `SquadTab` (elenco/olheiro/contratos no card), `ChampionsPanel`/`CopaBracket`, `SeasonJornal` (`jornal.tsx`), estádio (`estadio.tsx`), dinastia, Copa do Mundo (`copa-mundo.tsx`); **online**: `lobby.tsx` inteiro (etapa 5); **resto**: conta, álbum, ranking global, regras/dúvidas, bios das cartas (etapa 6 — perguntar ao Diego se traduz o humor das bios pelo sentido).
- 🧪 Como conferir sem o Vite recarregar a página no meio (achado hoje: editar arquivo durante o teste manda o jogo pro seletor de jogos): `npx vite preview --port 5195` servindo o `dist/` + `scratchpad/en-fim.mjs` apontando pra 5195.
- ⏳ Tudo no branch `claude/denis-save-file-x1osct`, esperando o "pode" do Diego (junto com Salão, pênaltis, caixa-preta e coroa).
...
- 🕐 **Segundo defeito, do mesmo dia**: *"tem sessão de pênaltis que mal começa e não deixa terminar, principalmente nas fases finais"*. A espera da fase contava só a disputa do SEU confronto (`isYou` em `copa-mundo.tsx`) — mas no mata-mata a tela mostra TODOS os confrontos da fase, então quando quem batia pênalti era outro a fase virava no meio da animação. Agora a espera cobre a disputa mais longa da fase, seja de quem for. (No relógio compartilhado da sala online isso já estava certo — o defeito era do relógio local: carreira, jogo rápido e sala sem o visual novo.)
- E o `pensRevealDelay` virou **piso seguro**: usa o pior caso das duas remontagens possíveis, nunca menos que 10 cobranças. A guarda `npm run penaltis` agora também confere que a espera da fase nunca é mais curta que a animação, em todos os placares.
...
- **Ficaram 3 partes** (`scripts/video-avatares-reels.mjs`, mesmo formato do `video-apresentacao-partes.mjs`: 1080×1920, cenas em CSS, Playwright grava, ffmpeg fecha o mp4, sem voz): 1) quem são as 156 lendas, camisa do clube E do ano, rosto vazio de propósito; 2) o pregão — envelope cego não mostra nada, a arte só nasce depois do martelo, carta de Lenda dourada; 3) o time montado — campinho, elenco no celular e time dos sonhos.
- **Prints reais** em `scripts/prints-avatares/` (ficam aqui e não em `src/escalacao/img/`: são do VÍDEO, não entram no bundle nem contam no teto de peso). Os do pregão saíram de uma partida rodada localmente com a prévia ligada; os três do time montado foram mandados pelo próprio Diego, da conta dele.
- **Messi Barcelona 2012 consertado**: o arquivo vinha com um retângulo preto opaco em vez de fundo transparente (era a última pendência de arte da prévia). Fundo recortado por componentes escuras coladas na borda + contorno preto de 2 px redesenhado na silhueta, pra ele ficar igualzinho aos outros 155. 21 KB, dentro do teto de 60 KB; `node scripts/check-legend-avatars.mjs` continua PASS com 156 identidades. Era o ÚNICO dos 156 com fundo opaco (varredura feita em todos).
- Reversão: `git checkout <commit anterior> -- public/avatars/lendas-v1/lionel-messi-barcelona-2012.webp` volta o arquivo antigo; apagar `scripts/video-avatares-reels.mjs` e `scripts/prints-avatares/` não mexe em NADA do jogo (é ferramenta de post). Nenhuma linha do jogo, do leilão, do online ou do banco foi tocada nesta entrega.
- 🚫 **O Diego cortou a tela do PESO** (11/09): a parte 3 terminava com "E NÃO PESA — cada retrato tem menos de 60 KB…". Palavras dele: *"não gostei de você falar isso aqui"*. Peso de arquivo é conversa nossa, não é papo pra tela do jogador. **Não pôr KB, bundle, lazy nem número de arquivo em vídeo/post nenhum.**
- 🎬 **VÍDEO DO ONLINE (11/09)**: ele não gostou da abertura genérica (*"não gostei"*) e pediu outra coisa: *"um vídeo dedicando que agora também está liberado no modo online as cartinhas com imagens das lendas... e aí eu junto com o vídeo que não coloquei"* (= a parte 3). Nasceu a parte `online` (`--so online`, 19s): anúncio do que mudou, o martelo com a cartinha de verdade, e o fecho "e o time fica assim 👇" emendando na parte 3. **Conferido antes de gravar**: `LEGEND_AVATARS_RELEASED = true` em `career-feature-release.ts`, e na sala online o `mesaOn` fica false e a revelação vira a MESMA cartinha com avatar — o anúncio é verdade.
...
iguais e ajustar os espaços"*. E em cima da v2 ele pegou mais um defeito: a zaga
ficava numa distância do goleiro DIFERENTE em cada formação (*"tem que ter um
...
**2. O pacote do campeão espera o toque** (`screens.tsx`) — começou como prop
`esperaToque` só no rápido. **Meia hora depois o Diego mandou valer em TODO modo**
(*"seja rápido offline, carreira ou qualquer outro"*), então o prop foi embora e o
relógio simplesmente DEIXOU DE EXISTIR:
- `CARD_PICK_SECONDS = 45` apagado dos DOIS arquivos que tinham cópia
  (`screens.tsx` e `careeronline.tsx`), junto com `deadline`/`now`/`remaining`,
  o badge de contagem e o prop `noTimer` (que só ligava/desligava esse relógio).
- Cobre de uma vez: rápido online, rápido offline, dinastia, carreira solo
  (liga/copa/supercopa, via `pyramidseason.tsx`), Copa do Mundo (`copa-mundo.tsx`)
  e a carreira ONLINE (`careeronline.tsx`, que escolhe entre 3 em vez de pacote).
- **Ninguém perde carta**: no pacote ela é gravada assim que a tela monta; na
  carreira online a garantia é a da SAÍDA (grava uma do time se fechar sem escolher)
  — que é exatamente o *"se ele sair aí conta automático"* que ele pediu.
📌 De quebra corrigi um comentário que mentia em `screens.tsx` (o do `COPA_GATE_S`,
que dizia segurar a gravação da carta — não segura desde 30/07, e agora nem existe
relógio de carta pra segurar).
