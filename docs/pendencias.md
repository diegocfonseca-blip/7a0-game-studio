...
- ⚠️ **O que AINDA está em PT** (etapa 6): **texto criativo gerado** — falas do pênalti (`PEN_INTRO/PEN_GO/...`), manchetes da carreira (`jornal.tsx`, 887 linhas), `historiaSondagem`, `eventoTituloBanner`, textos das comissões da agência (`r.texto` vem do store), `NOME_MSG` de `manto.ts`; **arquivos ainda não abertos**: `copa-mundo.tsx` (1331), `copa-mundo-online.tsx` (875), `dinastia.tsx` (2110), `ligahub.tsx` (667), `conta.tsx` (235 — o formulário "leve esse time pra carreira" que aparece no fim do rápido), `salao.tsx`, `careeronline.tsx`, `senha-nova.tsx`, e o que sobrar em `screens.tsx` (home, álbum, regras/dúvidas, apoio); **bios das cartas** (perguntar ao Diego se traduz o humor pelo sentido). 🏷️ Nome padrão de time "Meu Time" continua PT (é nome, não texto — decidir com o Diego se vira "My Team" pra quem cria em EN).
